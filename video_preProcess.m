...
for fil = 1:length(fileName)
   ...
   if COURTSHIP || LONGCHAMBER
      if any(size(fp.madFrame)<size(fp.arenaCrop))
         % this is probably wrong in some cases...
         fp.arenaCrop = fp.arenaCrop(1:size(fp.madFrame,1), 1:size(fp.madFrame,2));
      end
      frame = fp.getFrame(fp.initFrame);
      frame = fp.getForeGround(frame, 10);
      fp.samplesPerFly = 200*fp.nFlies;
      fp.clusterFrame(frame);
      if ismac || ispc
         clf
         imagesc(frame)
         fp.plotCluster(gmmInit)
         drawnow
      end
      save([trunk '_init'], 'fp','gmmInit')
   elseif PLAYBACK % automatically detect chambers
      clear fpc
      bw = fp.medianFrame>.8*mean(fp.medianFrame(:));
      if PLAYBACK % some post-processing to ensure we detect the whole chambers
