   ...
   methods (Access='public')
      ...
      function frame = getForeGround(obj, frame, sense, frameNumber)
         ...
         if ~isempty(obj.arenaCrop)
            frame = frame.*obj.arenaCrop(1:size(frame,1), 1:size(frame,2));               % delete everything outside arena        
         end
         frame = medfilt2(frame,[3 3]);
         
         frame0 = frame;
         frame = imclose(imopen(frame,obj.se2),obj.se5);
         % if the frame is empty after imclose revert (may mess up things)
         if all(frame(:)==0)
            frame = frame0;
         end
         
         frame = single(frame);
         frame(~reshape(obj.arenaCrop(1:size(frame,1), 1:size(frame,2)),1,[])) = 0;               % delete everything outside arena
      ...
      function [bw, frame, clusterGroup, Nconn] = getForeGroundAdaptive(obj, oriFrame, oldCentroids, sense,frameNumber)
         ...
         lostFly = 1;
         cnt = 0;
         if all(oldCentroids) == 0
            maskFrame = 3;
         else
            maskFrame = 1;
         end
         while ~isempty(lostFly) && cnt<10
            frame = obj.getForeGround(oriFrame, sense, frameNumber);
            senseCnt = 0;
            while max(frame(:))==0 && senseCnt<10
               warning('empty frame - increasing sensitivity')
               senseCnt = senseCnt+1;
               sense = 0.9*sense;
            end
            
            if maskFrame==1 % clean frame - remove specks that appear far away from old fly positions
            ...
            if maskFrame<3 % use old fly locations to identify flies with connected components
               % find isolated groups of flies 
               mask = imdilate(logical(frame), obj.seNew);% get mask from current frame
            else % otherwise fallback to: whole frame is one comp conn...
               mask = logical(true(size(frame)));
            end
            ...
            if ~isempty(lostFly)
               ...
               switch maskFrame
                  case 2
                     disp('   trying w/o masking.')
                  case 3
                     disp('   using whole frame.')
               end
               cnt = cnt+1;
            end
%                         subplot(221)
...
%                         drawnow
      ...
      function playTrack(obj, history, startFrame, plotFrame)
         if nargin==2
            startFrame = 1;
         end
         ...
         for t = startFrame:size(obj.tracks,1)-history
            if plotFrame
      ...
      function flyAng = getAnglePerFly(obj)
         % get angle of each fly
         flyAng = zeros(size(obj.tracks,1), obj.nFlies);
         for fly = 1:obj.nFlies
            flyAng(:, fly) = atan2(squeeze(obj.orientation(:,fly,2)), squeeze(obj.orientation(:,fly,1)));
         end
      end
      
      function flyAng = getAngleBetweenFlies(obj)
         % get angle between pairs of flies 
         warning('function untested - results are probably wrong!')
         flyAng = zeros(size(obj.tracks,1), obj.nFlies^2/2 - obj.nFlies/2);
         flyCnt = 0;
         for fly1 = 1:obj.nFlies
            u = squeeze(obj.orientation(:,fly1,:));  % x,y coordinates of FLY1
            u(:,3) = 0;                              % at trailing 0s size CROSS requires 3D vectors as input
            for fly2 = fly1+1:obj.nFlies
               flyCnt = flyCnt+1;
               v = squeeze(obj.orientation(:,fly2,:)); % x,y coordinates of FLY2
               v(:,3) = 0;                             % at trailing 0s size CROSS requires 3D vectors as input
               flyAng(:, flyCnt) = atan2(norm(cross(u',v')),dot(u',v'))';% this returns the angle between the vectors with good accuracy as per https://de.mathworks.com/matlabcentral/answers/101590-how-can-i-determine-the-angle-between-two-vectors-in-matlab
            end
         end
      end
